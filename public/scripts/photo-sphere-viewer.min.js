			...
			var addEvent=function(elt,evt,f){
				...
				var stayBetween=function(x,min,max){
					...
					var dist=function(x1,y1,x2,y2){
						...
						this.load=function(){container.innerHTML="";
						if(!!loading_html&&loading_html.nodeType===1)container.appendChild(loading_html);
							...
							if(!isCanvasSupported()){
								...
								return}
								if(window.THREE===undefined){console.log("PhotoSphereViewer: Three.js is not loaded.");
								return}viewer_size={width:0,height:0,ratio:0};
								if(readxmp&&!panorama.match(/^data:image\/[a-z]+;base64/))loadXMP();
								else createBuffer()};
								var getXMPData=function(file){
									var a=0,b=0;var data="";
								...
								return""};var getAttribute=function(data,attr){
									var a=data.indexOf("GPano:"+attr)+attr.length+8,b=data.indexOf('"',a);
									if(b==-1){a=data.indexOf("GPano:"+attr)+attr.length+7;b=data.indexOf("<",a)}
									...
									var loadXMP=function(){
										var xhr=null;if(window.XMLHttpRequest)xhr=new XMLHttpRequest;
											...
											else{container.textContent="XHR is not supported, update your browser!";
											return}xhr.onreadystatechange=function(){
												if(xhr.readyState==4&&xhr.status==200){var data=getXMPData(xhr.responseText);
												if(!data.length){createBuffer();
												return}pano_size={full_width:parseInt(getAttribute(data,"FullPanoWidthPixels")),
